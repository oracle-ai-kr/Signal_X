...
const SCR_ANAL_MARKET_DEFAULTS = {
  ...
  kr:     { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:39, sellTh:22, tpMult:6.5, slMult:0.8 },
  us:     { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:29, sellTh:10, tpMult:5.6, slMult:1.1 },
  crypto: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:33, sellTh:26, tpMult:7.1, slMult:0.6 }
};
...
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  kr: {
    on:  { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:39, sellTh:22, tpMult:6.5, slMult:0.8 },
    off: { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:39, sellTh:22, tpMult:6.5, slMult:0.8 }
  },
  us: {
    on:  { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:29, sellTh:10, tpMult:5.6, slMult:1.1 },
    off: { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:29, sellTh:10, tpMult:5.6, slMult:1.1 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:33, sellTh:26, tpMult:7.1, slMult:0.6 },
    off: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:33, sellTh:26, tpMult:7.1, slMult:0.6 }
  }
};
...
// [C] ⚠️ 데드코드 주의: 현재 호출처 0건 (sxRunBtEngine, scrQuickScore는 자체 clamp 사용).
//   - 이 헬퍼의 clamp 하한은 40 (buyTh) — 분석/BT 자체 clamp(20)와 다름.
//   - 잠재적 혼란 방지를 위해 향후 정리 권장. 다만 외부 호출 가능성 차단을 위해
//     v1 호환으로 export는 유지. 실제 사용은 SXE.regimeAdapt(regime).buyThAdj 직접 가산 권장.
...
function scrQuickScore(rows, tf) {
  ...
  const _sf = SXE._safetyFlags || {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,debtRatio:true,foreignSell:true,highBeta:true};
...
SXE._checkSafetyFilters = function(ind, rawScore, buyTh, volSoft, tf){
  if(!ind) return { pass: true, reason: '' };
  // [S264] 폴백 기본값 10개 전수 ON. foreignSell은 ind._foreignConsecSell != null 가드로 KIS 미연결 자동 통과.
  const _sf = SXE._safetyFlags || {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,debtRatio:true,foreignSell:true,highBeta:true};

  // 1. 임계값 마진 부족 (rawScore가 buyTh 턱걸이일 때)
  if(_sf.threshold && rawScore < buyTh + 2) return { pass: false, reason: '🔒임계값' };
  ...
  // 6. MA60 저항 임박
  if(_sf.ma60resist && ind.maAlign && ind.maAlign.ma60 != null && ind.price < ind.maAlign.ma60){
    const d60 = ((ind.maAlign.ma60 - ind.price) / ind.price) * 100;
...
SXE._btEarlyExit = (function(){
  ...
  //   주의: sx_screener.html의 _eeEnsureDefaults + resetEarlyExit과 반드시 같이 수정.
  return {
    enabled:   true,   // 마스터 스위치
    score:     true,   // rawScore <= sellTh
    ...
    trailMode: 'profit', // 'immediate' = 진입 즉시 트레일링 / 'profit' = 수익권 진입 후 작동
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
    ...
    if (!pos && !pendingSignal) {
      if (rawScore >= buyTh && _scrMomOscPass(mom, osc, tf)) {
        ...
        if(SXE._applySafetyToBt){
          const _safRes = SXE._checkSafetyFilters(ind, rawScore, buyTh, volSoft, tf);
      ...
      if(SXE._btEarlyExit && SXE._btEarlyExit.enabled){
        ...
        if(ee.breakeven && pos.peak >= pos.entry * (1 + atrPctNow * ee.beTrigger)){
        ...
        if(ee.trailing){
          ...
          if(_mode === 'profit'){
            const _trailGate = pos.entry * (1 + atrPctNow * (ee.trailTrigger || 1.0));
            _trailActive = (pos.peak >= _trailGate);
          }
          if(_trailActive){
            const trailSL = pos.peak * (1 - atrPctNow * ee.trailMult);
            if(trailSL > pos.sl) pos.sl = trailSL;
          }
        }
      }

      // ── 청산 판정 (TP/SL 기존 로직 — 위에서 SL이 동적으로 끌어올려졌을 수 있음) ──
      if (c >= pos.tp) {
        ...
        trades.push({ entry: pos.entry, exit: exitP, pnl: +pnl.toFixed(2), type: _exitType, exitReason: _exitReason, bars: i - pos.entryIdx, entryIdx: pos.entryIdx, exitIdx: i, entryDate: rows[pos.entryIdx]?.date||'', exitDate: rows[i]?.date||'', tp: pos.tp, sl: pos.sl });
        pos = null;
        ...
        if(_earlyExit){
          ...
          trades.push({ entry: pos.entry, exit: exitP, pnl: +pnl.toFixed(2), type: _exitType, exitReason: _earlyExit, bars: i - pos.entryIdx, entryIdx: pos.entryIdx, exitIdx: i, entryDate: rows[pos.entryIdx]?.date||'', exitDate: rows[i]?.date||'', tp: pos.tp, sl: pos.sl });
          pos = null;
        }
      }
    }
  ...
  return { winRate, profitFactor: pf, totalPnl, mdd: +maxDD.toFixed(2), totalTrades: closed, avgWin, avgLoss, maxConsecLoss, scores: _btScores.map(s => s.t), scores3: _btScores, trades, rowsLength: rows.length, gateBlocks: _gateBlocks, gateReasons: _gateReasons };
}
...
// [S264] 기본값 10개 전수 ON 정책. 스크리너 _safetyDefaults와 권위 동기화.
//        주의: 스크리너 _loadSafetyFilterUI가 첫 로드 시 localStorage 값으로 덮어씀 — 이 초기값은
//        스크리너 로드 전 BT/실시간 신호가 안전필터 호출하는 레이스 윈도우에서만 사용됨.
SXE._safetyFlags = {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,debtRatio:true,foreignSell:true,highBeta:true};
