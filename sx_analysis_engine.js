...
const Candle = {
  analyze(rows) {
    ...
    // 스피닝탑: 몸통 작고(10~30%), 위아래 꼬리 비슷
    if (range > 0 && absB / range >= 0.1 && absB / range <= 0.3) {
      const upShadow = r.high - Math.max(r.open, r.close);
      const dnShadow = Math.min(r.open, r.close) - r.low;
      if (upShadow > 0 && dnShadow > 0 && Math.min(upShadow, dnShadow) / Math.max(upShadow, dnShadow) > 0.4)
