// ════════════════════════════════════════════════════════════
//  SIGNAL X — Analysis Engine v1.8
//  공용 분석 모듈 (sx_screener / sx_backtester 공유)
//  13 지표모듈 + 5 기타모듈 + 7 고급엔진 + 점수산출 + 안전필터 + 스마트필터
//  S28: SX_CUSTOM_THRESHOLDS 통합 + sxAdaptRows + sxRunBtEngine
...
const Candle = {
  analyze(rows) {
    ...
    const continuation = patterns.filter(pt => ['적삼병','흑삼병','갭상승','갭하락'].includes(pt.name)).map(pt => pt.name);
...
// ════════════════════════════════════════════════════════════
//  시장환경 판정 (MarketEnv) — S45
//  1차: ORACLE_MARKET_ENV (market_signal 종합 환경)
//  2차: SX_MARKET_INDEX (스크리너가 스캔 시작 시 fetch)
//  시장별 지수 매핑:
...
const MarketEnv = {
  ...
  /**
   * 시장환경 데이터 로드 (5분 캐시)
   * 우선순위: _liveIndex > SX_MARKET_INDEX > ORACLE_MARKET_ENV > fallback
   */
  load() {
    ...
    const result = {
      ...
      btc: null,
      oracleEnv: null, // market_signal 종합
      overall: { state: 'neutral', score: 0 }
    };
    try {
      // ── 1) ORACLE_MARKET_ENV (market_signal 종합 환경) ──
      try {
        ...
        if (raw) {
          ...
          if (age < 24 * 3600 * 1000) {
            result.oracleEnv = env;
          }
        }
      } catch (_) {}
      // ── 2) 실시간 지수 데이터 (스크리너 fetch or localStorage) ──
      ...
      if (!live) {
        try {
          ...
          if (saved) {
            ...
            if (d._ts && Date.now() - d._ts < 3600 * 1000) live = d;
          }
        } catch (_) {}
      }
      const classify = (cr) => {
      ...
      // ── 3) ORACLE_MARKET_ENV sectors fallback ──
      if (result.oracleEnv && result.oracleEnv.sectors) {
        const sec = result.oracleEnv.sectors;
        // equity sector가 있으면 해당 점수로 국내/해외 판정 보강
        if (!result.kospi && !result.kosdaq && sec.equity) {
          const eqScore = sec.equity.score;
          const eqDir = eqScore >= 55 ? 'bull' : eqScore >= 45 ? 'mild_bull' : eqScore <= 35 ? 'bear' : eqScore <= 45 ? 'mild_bear' : 'neutral';
          result.kospi = { name: 'KOSPI(env)', changeRate: null, close: 0, dir: eqDir, score: classify(eqDir === 'bull' ? 2 : eqDir === 'bear' ? -2 : 0).score };
        }
        if (!result.btc && sec.crypto) {
          const crScore = sec.crypto.score;
          const crDir = crScore >= 55 ? 'bull' : crScore >= 45 ? 'mild_bull' : crScore <= 35 ? 'bear' : crScore <= 45 ? 'mild_bear' : 'neutral';
          result.btc = { name: 'BTC(env)', changeRate: null, close: 0, dir: crDir, score: classify(crDir === 'bull' ? 2 : crDir === 'bear' ? -2 : 0).score };
        }
      }
      // ── 4) 종합 점수 ──
      const allScores = [
      ...
      // ORACLE_MARKET_ENV grade로 보정
      let ovrState = 'neutral';
      if (result.oracleEnv) {
        const g = result.oracleEnv.grade;
        if (g === 'EXTREME') ovrState = avgScore >= 0 ? 'mild_bear' : 'bear';
        else if (g === 'CAUTION') ovrState = avgScore > 0 ? 'mild_bull' : 'mild_bear';
        else ovrState = avgScore > 0.8 ? 'bull' : avgScore > 0.2 ? 'mild_bull' : avgScore < -0.8 ? 'bear' : avgScore < -0.2 ? 'mild_bear' : 'neutral';
      } else {
        ovrState = avgScore > 0.8 ? 'bull' : avgScore > 0.2 ? 'mild_bull' : avgScore < -0.8 ? 'bear' : avgScore < -0.2 ? 'mild_bear' : 'neutral';
      }
      result.overall = { state: ovrState, score: avgScore, oracleGrade: result.oracleEnv?.grade || null, oracleScore: result.oracleEnv?.overallScore || null };
  ...
  getWeight(market) {
    ...
    if (market === 'coin') {
      ...
      // 해외: S&P500 + NASDAQ 가중 평균
      const scores = [env.sp500?.score, env.nasdaq?.score, env.dow?.score].filter(v => v != null);
      if (scores.length) {
        const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
        return clamp(avg * 4, -8, 8);
      }
      // fallback: ORACLE_MARKET_ENV equity sector
      if (env.oracleEnv?.sectors?.equity) {
        const s = env.oracleEnv.sectors.equity.score;
        return clamp(((s - 50) / 25) * 8, -8, 8);
      }
      return 0;
    } else {
    ...
    if (!primary || primary.dir === 'unknown') return 0;
    return clamp(primary.score * 4, -8, 8);
  },
  /** 조건검색용 상태 판정 */
  getState(market) {
    const env = this.load();
  ...
  getSummary(market) {
    ...
    const envAge = env.oracleEnv ? Math.round((Date.now() - new Date(env.oracleEnv.date).getTime()) / 60000) : null;
    const envAgeStr = envAge != null ? (envAge < 60 ? `${envAge}분전` : `${Math.round(envAge / 60)}시간전`) : null;
    const oracleStr = env.oracleEnv ? `${env.oracleEnv.grade} ${env.oracleEnv.overallScore}점` : null;
    const indices = [];
    ...
    return { state, stateLabel, oracleStr, envAgeStr, indices };
  }
};
...
function scrComputeScore(ind, volSoft, ctxBonus) {
  ...
  const combined = clamp(s + b * 0.5 + clamp(ctxBonus, -35, 35) * 0.8 + aux + _mktW, 0, 100);
