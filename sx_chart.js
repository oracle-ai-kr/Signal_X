...
function drawGrid(ctx, pad, W, H, lo, hi, lines){
  ...
  for(var i=0;i<=lines;i++){
    ...
    ctx.fillText(v>=1000?Math.round(v).toLocaleString():v.toFixed(1), W-pad.r+30, y+3);
  }
}
...
function drawCandles(ctx, data, pad, cw, yFn, thick){
  ...
  data.forEach(function(d,i){
    ...
    ctx.strokeStyle=bull?BULL:BEAR; ctx.fillStyle=bull?BULL:BEAR;
    ctx.beginPath(); ctx.moveTo(x,yFn(d.high)); ctx.lineTo(x,yFn(d.low));
    ctx.lineWidth=thick?0.8:0.7; ctx.stroke();
    var bTop=yFn(Math.max(d.open,d.close)), bBot=yFn(Math.min(d.open,d.close));
    var bH=Math.max(bBot-bTop, thick?0.8:0.5), bw=Math.max(cw*(thick?0.6:0.55), thick?1.5:1);
    if(bull) ctx.strokeRect(x-bw/2,bTop,bw,bH);
...
function drawMALegend(ctx, closes, pad, H, fontSize){
  ...
  [5,10,20,60,120].forEach(function(p){
    ...
    ctx.fillText('MA'+p, lx, H-(fontSize||8)+3);
    lx+=ctx.measureText('MA'+p).width+6;
  });
  ctx.fillStyle=BB_MID;
  ctx.fillText('BB', lx, H-(fontSize||8)+3);
}
...
function drawMini(canvasId, rows){
  ...
  if(!canvas) return;
  var W = canvas.clientWidth||canvas.parentElement.clientWidth||360;
  var H = 160;
...
function openFull(rows, stockName){
  ...
  if(!ov){
    ...
    ov.style.cssText = 'position:fixed;inset:0;background:var(--bg,#fff);z-index:500;display:flex;flex-direction:column;overflow:hidden';
    document.body.appendChild(ov);
  }
  ov.style.display = 'flex';

  var data = rows.slice(-80);
  var closes = data.map(function(d){return d.close;});
  ...
  ov.innerHTML =
    '<div style="display:flex;align-items:center;padding:10px 12px;border-bottom:1px solid var(--border,#eee);flex-shrink:0">' +
      '<button onclick="SXChart.closeFull()" style="border:none;background:none;font-size:16px;cursor:pointer;color:var(--text,#333);padding:4px 8px">\u2039</button>' +
      '<span style="font-size:13px;font-weight:700;flex:1;text-align:center;color:var(--text,#333)">' + (stockName||'') + ' 상세 차트</span>' +
      '<span style="width:32px"></span>' +
    '</div>' +
    '<div style="flex:1;overflow-y:auto;padding:8px" id="sxChartBody">' +
      '<div style="font-size:10px;font-weight:600;color:var(--text2,#888);margin-bottom:4px">캔들 + MA + BB (최근 ' + data.length + '봉)</div>' +
      '<canvas id="sxFullCandle" style="width:100%;height:220px;display:block;background:var(--surface,#fff);border:1px solid var(--border,#eee);border-radius:8px"></canvas>' +
      '<div style="font-size:10px;font-weight:600;color:var(--text2,#888);margin:10px 0 4px">거래량</div>' +
      '<canvas id="sxFullVol" style="width:100%;height:60px;display:block;background:var(--surface,#fff);border:1px solid var(--border,#eee);border-radius:8px"></canvas>' +
      '<div style="font-size:10px;font-weight:600;color:var(--text2,#888);margin:10px 0 4px">RSI (14)</div>' +
      '<canvas id="sxFullRSI" style="width:100%;height:80px;display:block;background:var(--surface,#fff);border:1px solid var(--border,#eee);border-radius:8px"></canvas>' +
      '<div style="font-size:10px;font-weight:600;color:var(--text2,#888);margin:10px 0 4px">MACD (12, 26, 9)</div>' +
      '<canvas id="sxFullMACD" style="width:100%;height:80px;display:block;background:var(--surface,#fff);border:1px solid var(--border,#eee);border-radius:8px"></canvas>' +
    '</div>';

  requestAnimationFrame(function(){
    _drawFullCandle('sxFullCandle', data, closes, bb, 220);
    _drawFullVolume('sxFullVol', data, 60);
    _drawFullRSI('sxFullRSI', rsiArr, 80);
    _drawFullMACD('sxFullMACD', macdObj, 80);
  });
}
...
function _drawFullVolume(id, data, H){
  ...
  ctx.fillText(maxVol>=1e6?(maxVol/1e6).toFixed(0)+'M':maxVol>=1e3?(maxVol/1e3).toFixed(0)+'K':maxVol.toFixed(0), W-4, pad.t+8);
}
...
function _drawFullMACD(id, macdObj, H){
  ...
  ctx.fillText(absMax.toFixed(1), W-4, pad.t+8);
  ctx.fillText((-absMax).toFixed(1), W-4, H-pad.b-2);
}
