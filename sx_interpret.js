// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v2.3
...
SXI.advMarketEnv = function(state, stateLabel, indices, oracleStr){
  if(!state || state==='unknown') return null;
  const bull = state.includes('bull'), bear = state.includes('bear');
  const idxStr = indices&&indices.length ? indices.join(' · ') : '지수 데이터 없음';
  const oStr = oracleStr ? ' · ORACLE 종합: '+oracleStr : '';
  if(bull && state.includes('strong')){
    return {tone:'bullish',label:'강한 강세장',text:`${idxStr}${oStr}. 시장 전반이 강한 상승 흐름입니다. 매수 신호의 신뢰도가 크게 높아지며, 대부분의 종목이 시장 베타를 따라 상승하는 구간입니다. 눌림목 매수·추세 추종 전략이 가장 효과적이며, 약세 종목도 시장 분위기에 끌려 반등이 나올 수 있습니다. 다만 과열 구간에 접어들면 일시 조정이 빠르게 오므로, 개별 종목의 과매수 지표(RSI 80+, BB 상단 이탈)를 함께 점검하세요.`};
  }
  if(bull){
    return {tone:'bullish',label:'강세',text:`${idxStr}${oStr}. 시장 환경이 우호적이어서 매수 신호의 신뢰도가 높아집니다. 정배열 종목의 눌림목 매수, 거래량 동반 돌파 전략이 유리하며, 약간의 기술적 조정은 매수 기회로 활용할 수 있습니다. 시장 전체가 양호하더라도 업종별 차별화는 존재하므로, 개별 종목의 모멘텀과 수급을 반드시 확인하세요.`};
  }
  if(bear && state.includes('strong')){
    return {tone:'danger',label:'강한 약세장',text:`${idxStr}${oStr}. 시장 전반이 강한 하락 압력을 받고 있습니다. 대부분의 매수 신호가 무력화되기 쉬우며, 기술적 반등도 짧고 약하게 끝나는 경우가 많습니다. 현금 비중을 높이고, 기존 보유 종목은 손절 기준을 타이트하게 관리하세요. 신규 매수는 시장 반등 신호(지수 양봉 + 거래량 급증 + VIX 하락)가 확인된 후에만 소액으로 접근하는 것이 안전합니다.`};
  }
  if(bear){
    return {tone:'bearish',label:'약세',text:`${idxStr}${oStr}. 시장 환경이 비우호적이어서 매수 신호에 대한 보수적 접근이 필요합니다. 강한 개별 모멘텀이 있는 종목만 선별적으로 접근하되, 진입 비중을 평소의 50~70%로 줄이고 손절 기준을 엄격히 적용하세요. 시장 약세기에는 방어주·고배당주·역발상 매수보다 현금 확보가 최우선이며, 추세 반전 신호가 나올 때까지 관망하는 것도 좋은 전략입니다.`};
  }
  return {tone:'neutral',label:'중립',text:`${idxStr}${oStr}. 시장 전반의 환경이 중립적이어서 개별 종목의 기술적 신호에 집중해야 합니다. 지수 방향이 명확하지 않을 때는 업종·테마 순환에 따라 종목별 차별화가 심해지므로, 시장 전체 흐름보다 개별 종목의 수급·모멘텀·차트 구조를 더 비중 있게 판단하세요. 양방향 시나리오를 모두 준비하고, 핵심 지지/저항 이탈 시에만 대응하는 전략이 효율적입니다.`};
};
