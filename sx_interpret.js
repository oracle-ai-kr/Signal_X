// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v3.3
...
SXI.compositeMomentum = function(ind){
  ...
  if(psyV!=null && psyV>=80 && rsi>=70) notes.push({tone:'danger',icon:'🔴',title:'심리도 + RSI 이중 과열',text:`심리도 ${psyV.toFixed(0)}% + RSI ${rsi.toFixed(0)} — 투자 심리와 가격 모멘텀 모두 과열 상태입니다. 역사적으로 이 조합에서는 급조정 확률이 매우 높으며, 추격 매수는 절대 금물입니다. 분할 익절을 강력히 검토하세요.`});
  if(psyV!=null && psyV<=20 && rsi<=30) notes.push({tone:'bullish',icon:'🟢',title:'심리도 + RSI 이중 과매도',text:`심리도 ${psyV.toFixed(0)}% + RSI ${rsi.toFixed(0)} — 투자 심리와 가격 모멘텀 모두 극단적 침체 구간입니다. 매도 에너지가 거의 소진되었으며, 기술적 반등 가능성이 높습니다. 거래량 증가·양봉 확인 후 분할 매수를 시작할 수 있는 구간입니다.`});
...
SXI.compositeRisk = function(ind){
  ...
  if(abR!=null && abR>=1.8 && psyR!=null && psyR>=80) notes.push({tone:'danger',icon:'🔴',title:'AB Ratio 극단 매수 + 심리도 과열',text:`AB Ratio ${abR.toFixed(2)}(강한 매수세) + 심리도 ${psyR.toFixed(0)}%(과열) — 매수 열기가 극에 달했습니다. 장중 고가를 적극적으로 높이는 매수세와 과도한 낙관이 겹쳐, 급격한 차익실현 매물이 쏟아질 위험이 있습니다.`});
  if(abR!=null && abR<=0.55 && psyR!=null && psyR<=20) notes.push({tone:'bullish',icon:'🟢',title:'AB Ratio 극단 매도 + 심리도 침체',text:`AB Ratio ${abR.toFixed(2)}(강한 매도세) + 심리도 ${psyR.toFixed(0)}%(과매도) — 매도 에너지가 극에 달해 소진 직전입니다. 양봉 + 거래량 증가가 나타나면 바닥 반전 가능성이 높으며, 분할 매수 시작 구간으로 검토할 수 있습니다.`});
  if(abT==='bearish' && rsi>=70) notes.push({tone:'warning',icon:'⚠️',title:'AB 매도 우위인데 RSI 과매수',text:'가격은 아직 높지만 장중 매도 압력이 매수를 압도하고 있습니다. 고점에서의 물량 분산가 시작되었을 가능성이 있으며, 윗꼬리 음봉 출현에 주의하세요.'});
  return notes;
};
...
SXI.compositeSynergy = function(ind){
  ...
  if(psyV!=null && psyV>=75 && abT==='bearish')
    notes.push({tone:'danger',icon:'🔴',title:'심리도 과열 + AB 매도 전환 → 고점 분배',text:`심리도 ${psyV.toFixed(0)}%(과매수) 상태에서 AB Ratio가 매도 우위로 전환되었습니다. 과열된 낙관 속에서 장중 매도 압력이 커지고 있어, 세력(큰손)이 물량을 던지기 시작했을 가능성이 있습니다. 분할 익절을 강력히 검토하세요.`});
...
SXI.relatedKeywords = function(stock, indicators, qs){
  ...
  if(sector){
    ...
    kws.add(`${sector} 대장주`);
    kws.add(`${sector} 전망`);
    // 섹터 테마 풀에서 랜덤 5~8개 선택
    const themePool = SXI._findThemePool(sector, name);
    themePool.forEach(t => kws.add(t));
  }

  // 3. 스마트태그 기반 키워드
  tags.forEach(t => {
    const label = t.label || '';
    const pool = SXI._signalKeywords[label];
    if(pool) pool.forEach(k => kws.add(k));
    // 태그 라벨 자체도 검색어로
    if(label.includes('다이버')) kws.add('다이버전스 종목');
    if(label.includes('정배열')) kws.add('이평선 정배열 종목');
    if(label.includes('Vol')) kws.add('거래량 특이 종목');
  });

  // 4. 시가총액 규모 키워드
  const scale = mc >= 100000 ? 'large' : mc >= 10000 ? 'mid' : mc >= 1000 ? 'small' : 'micro';
  (SXI._scaleKeywords[scale] || []).forEach(k => kws.add(k));

  // 5. 매매 신호 키워드
  (SXI._actionKeywords[action] || []).forEach(k => kws.add(k));

  // 6. 추세/레짐 키워드
  const trendType = trendPct > 5 ? 'uptrend' : trendPct < -5 ? 'downtrend' : 'sideways';
  (SXI._trendKeywords[trendType] || []).forEach(k => kws.add(k));

  // 7. 지표 상태 키워드
  if(rsi > 70) { kws.add('RSI 과매수'); kws.add('과열 종목'); kws.add('단기 조정 주의'); kws.add('차익실현 구간'); }
  else if(rsi < 30) { kws.add('RSI 과매도'); kws.add('반등 기대주'); kws.add('저점 매수 기회'); kws.add('낙폭 과대'); }
  if(adx > 30) { kws.add('강한 추세'); kws.add('추세 강도 상위'); kws.add('ADX 강세'); kws.add('추세 추종'); }
  else if(adx < 15) { kws.add('비추세 구간'); kws.add('횡보 종목'); kws.add('방향성 부재'); }
  if(indicators?.bb?.isSqueeze || indicators?.squeeze?.squeeze) { kws.add('변동성 수축'); kws.add('스퀴즈 종목'); kws.add('돌파 임박'); }

  // 8. 레짐 키워드
  if(regime){
    if(regime.label==='추세장') { kws.add('추세장 종목'); kws.add('모멘텀 매매'); kws.add('추세 추종 전략'); }
    else if(regime.label==='횡보장') { kws.add('횡보장 매매'); kws.add('박스권 돌파'); kws.add('레인지 전략'); }
    else if(regime.label==='약세장') { kws.add('약세장 방어'); kws.add('하방 리스크 관리'); kws.add('현금 비중'); }
  }

  // 9. 시장 구분 키워드
  if(market==='KOSPI') { kws.add('코스피'); kws.add('코스피 종목'); }
  else if(market==='KOSDAQ') { kws.add('코스닥'); kws.add('코스닥 종목'); kws.add('코스닥 성장주'); }

  // 10. 종목명 파생 (업종 추정)
  const nameKws = SXI._deriveFromName(name);
  nameKws.forEach(k => kws.add(k));

  // 11. 캔들 패턴 키워드
  const patterns = indicators?.patterns || indicators?._advanced?.patterns || null;
  if(patterns){
    const patList = Array.isArray(patterns) ? patterns : (patterns.list || []);
    patList.forEach(p => {
      const pid = typeof p==='string' ? p : (p.id||p.name||'');
      const pool = SXI._candleKeywords[pid];
      if(pool) pool.forEach(k => kws.add(k));
    });
  }
  ...
  if(trendPct > 5 && adx > 25){
    (SXI._techniqueKeywords.breakout || []).slice(0,3).forEach(k => kws.add(k));
    (SXI._techniqueKeywords.pullback || []).slice(0,3).forEach(k => kws.add(k));
  } else if(trendPct < -5){
    (SXI._techniqueKeywords.swing || []).slice(0,3).forEach(k => kws.add(k));
    (SXI._techniqueKeywords.divergence || []).slice(0,3).forEach(k => kws.add(k));
  } else {
    (SXI._techniqueKeywords.channel || []).slice(0,3).forEach(k => kws.add(k));
    (SXI._techniqueKeywords.swing || []).slice(0,3).forEach(k => kws.add(k));
  }

  // 13. 수급/투자자 키워드
  const fr = stock.foreignRatio || 0;
  if(fr > 30) (SXI._investorKeywords.foreign_buy || []).forEach(k => kws.add(k));
  else if(fr > 10) { kws.add('외국인 보유 비중'); kws.add('외국인 동향'); }
  else { (SXI._investorKeywords.retail || []).slice(0,3).forEach(k => kws.add(k)); }

  // 14. 재무/밸류에이션 키워드
  if(action==='BUY') (SXI._valuationKeywords.value || []).slice(0,4).forEach(k => kws.add(k));
  if(mc > 50000) (SXI._valuationKeywords.dividend || []).slice(0,3).forEach(k => kws.add(k));
  (SXI._valuationKeywords.event || []).slice(0,4).forEach(k => kws.add(k));

  // 15. 차트 분석 키워드
  (SXI._chartKeywords.support || []).slice(0,3).forEach(k => kws.add(k));
  (SXI._chartKeywords.resistance || []).slice(0,3).forEach(k => kws.add(k));
  if(trendPct > 3 || trendPct < -3) (SXI._chartKeywords.trendline || []).slice(0,2).forEach(k => kws.add(k));
  // 차트 패턴 키워드 2~3개 랜덤
  const cpArr = (SXI._chartKeywords.pattern || []).slice().sort(()=>Math.random()-0.5);
  cpArr.slice(0,3).forEach(k => kws.add(k));
...
SXI.advFundamental = function(fin, price){
  ...
  if(fin.roa != null){
    if(fin.roa >= 10){
      lines.push(`ROA ${fin.roa.toFixed(1)}% — 총자산 대비 수익 창출력이 매우 우수합니다. 자산 효율성이 높아 투하자본 대비 이익이 탁월합니다.`);
      bullCount += 1;
    } else if(fin.roa >= 5){
      lines.push(`ROA ${fin.roa.toFixed(1)}% — 양호한 자산수익률입니다. 자본 활용 효율이 적절한 수준입니다.`);
    } else if(fin.roa >= 0){
      lines.push(`ROA ${fin.roa.toFixed(1)}% — 낮은 자산수익률입니다. 대규모 설비투자 업종이 아니라면 수익성 개선이 필요합니다.`);
    } else {
      lines.push(`ROA ${fin.roa.toFixed(1)}% — 자산 대비 순손실 상태입니다. 수익 구조 개선이 시급합니다.`);
      bearCount += 1;
    }
  }
  ...
  if(fin.operatingMargin != null){
    if(fin.operatingMargin >= 20){
      lines.push(`영업이익률 ${fin.operatingMargin.toFixed(1)}% — 높은 마진율로, 본업의 수익 구조가 탄탄합니다. 가격 결정력이나 원가 경쟁력이 있는 기업입니다.`);
      bullCount += 1;
    } else if(fin.operatingMargin >= 8){
      lines.push(`영업이익률 ${fin.operatingMargin.toFixed(1)}% — 적정 수준의 마진입니다. 업종 평균과 비교하여 판단하세요.`);
    } else if(fin.operatingMargin >= 0){
      lines.push(`영업이익률 ${fin.operatingMargin.toFixed(1)}% — 박리다매형이거나 비용 구조 개선이 필요합니다.`);
    } else {
      lines.push(`영업이익률 ${fin.operatingMargin.toFixed(1)}% — 영업적자 상태입니다. 매출 확대나 비용 절감 없이는 지속 불가능합니다.`);
      bearCount += 1;
    }
  }
  ...
  if(fin.netMargin != null){
    if(fin.netMargin >= 15){
      lines.push(`순이익률 ${fin.netMargin.toFixed(1)}% — 최종 수익성이 매우 높습니다. 영업외 비용이 적고 효율적인 비용 관리가 이루어지고 있습니다.`);
    } else if(fin.netMargin >= 0){
      lines.push(`순이익률 ${fin.netMargin.toFixed(1)}% — 흑자를 유지하고 있습니다.`);
    } else {
      lines.push(`순이익률 ${fin.netMargin.toFixed(1)}% — 최종 순손실 상태입니다.`);
    }
  }

  // ── S60: EBITDA 마진 (근사치) ──
  if(fin.ebitdaMargin != null){
    if(fin.ebitdaMargin >= 25){
      lines.push(`EBITDA 마진 ${fin.ebitdaMargin.toFixed(1)}% — 현금 창출력이 우수합니다 (감가상각 미분리 근사치).`);
      bullCount += 1;
    } else if(fin.ebitdaMargin < 0){
      lines.push(`EBITDA 마진 ${fin.ebitdaMargin.toFixed(1)}% — 영업 현금흐름이 음수입니다 (근사치).`);
    }
  }
  ...
  if(fin.psr != null){
    if(fin.psr <= 1){
      lines.push(`PSR ${fin.psr.toFixed(2)}배 — 매출 대비 극저평가입니다. 수익성이 개선되면 주가 재평가 가능성이 높습니다.`);
      bullCount += 1;
    } else if(fin.psr <= 3){
      lines.push(`PSR ${fin.psr.toFixed(2)}배 — 매출 대비 적정 수준입니다.`);
    } else if(fin.psr <= 10){
      lines.push(`PSR ${fin.psr.toFixed(2)}배 — 매출 대비 다소 높습니다. 고성장이 지속되는지 확인이 필요합니다.`);
    } else {
      lines.push(`PSR ${fin.psr.toFixed(2)}배 — 매출 대비 고평가 구간입니다. 실적 성장이 뒷받침되지 않으면 밸류에이션 부담이 큽니다.`);
      bearCount += 1;
    }
  }
  ...
  if(fin.evEbitda != null){
    if(fin.evEbitda > 0 && fin.evEbitda <= 6){
      lines.push(`EV/EBITDA ${fin.evEbitda.toFixed(1)}배 — 기업가치 대비 저평가입니다. M&A 매력이 있는 구간입니다 (현금 미반영 근사치).`);
      bullCount += 1;
    } else if(fin.evEbitda <= 12){
      lines.push(`EV/EBITDA ${fin.evEbitda.toFixed(1)}배 — 적정 수준입니다 (근사치).`);
    } else if(fin.evEbitda <= 25){
      lines.push(`EV/EBITDA ${fin.evEbitda.toFixed(1)}배 — 다소 높은 밸류에이션입니다 (근사치).`);
    } else {
      lines.push(`EV/EBITDA ${fin.evEbitda.toFixed(1)}배 — 고평가 구간입니다 (근사치).`);
      bearCount += 1;
    }
  }
  ...
  if(fin.bps != null && price > 0){
    ...
    if(bpsRatio < 0.7){
      lines.push(`BPS ${Math.round(fin.bps).toLocaleString()}원 (PBR ${bpsRatio.toFixed(2)}배) — 장부가치 대비 큰 폭 할인 거래 중입니다.`);
    } else if(bpsRatio > 3){
      lines.push(`BPS ${Math.round(fin.bps).toLocaleString()}원 — 주가가 장부가치의 ${bpsRatio.toFixed(1)}배로, 무형자산 프리미엄이 큽니다.`);
    }
  }

  // ── S60: PCR (근사치) ──
  if(fin.pcr != null){
    if(fin.pcr > 0 && fin.pcr <= 5){
      lines.push(`PCR ${fin.pcr.toFixed(1)}배 — 현금흐름 대비 저평가입니다 (영업이익 대용 근사치).`);
      bullCount += 1;
    } else if(fin.pcr <= 15){
      lines.push(`PCR ${fin.pcr.toFixed(1)}배 — 적정 수준입니다 (근사치).`);
    } else if(fin.pcr > 15){
      lines.push(`PCR ${fin.pcr.toFixed(1)}배 — 현금흐름 대비 고평가입니다 (근사치).`);
    }
  }

  // ── S60: PEG ──
  if(fin.peg != null){
    if(fin.peg > 0 && fin.peg <= 0.8){
      lines.push(`PEG ${fin.peg.toFixed(2)} — 성장성 대비 극저평가입니다. 이익 성장이 지속된다면 매우 매력적인 밸류에이션입니다.`);
      bullCount += 2;
    } else if(fin.peg <= 1.2){
      lines.push(`PEG ${fin.peg.toFixed(2)} — 성장성 대비 적정~저평가입니다. PER이 이익 성장률로 정당화되는 수준입니다.`);
      bullCount += 1;
    } else if(fin.peg <= 2.0){
      lines.push(`PEG ${fin.peg.toFixed(2)} — 성장성 대비 보통입니다. 이익 성장 둔화 시 밸류에이션 부담이 될 수 있습니다.`);
    } else {
      lines.push(`PEG ${fin.peg.toFixed(2)} — 성장성 대비 고평가입니다. PER이 이익 성장률을 크게 초과하고 있어, 실적 미달 시 하방 위험이 큽니다.`);
      bearCount += 1;
    }
  }
  ...
  if(fin.currentRatio != null){
    if(fin.currentRatio >= 300){
      lines.push(`유동비율 ${fin.currentRatio.toFixed(0)}% — 단기 지급능력이 매우 충분합니다 (총자산/총부채 근사치).`);
      bullCount += 1;
    } else if(fin.currentRatio >= 150){
      lines.push(`유동비율 ${fin.currentRatio.toFixed(0)}% — 적정 수준입니다 (근사치).`);
    } else if(fin.currentRatio >= 100){
      lines.push(`유동비율 ${fin.currentRatio.toFixed(0)}% — 유동성이 다소 빡빡합니다 (근사치). 단기 부채 상환 여력을 점검하세요.`);
    } else {
      lines.push(`유동비율 ${fin.currentRatio.toFixed(0)}% — 자산보다 부채가 많습니다 (근사치). 유동성 위기 가능성에 주의하세요.`);
      bearCount += 1;
    }
  }
  ...
  if(fin.epsGrowth != null){
    if(fin.epsGrowth >= 50){
      lines.push(`EPS 성장률 +${fin.epsGrowth.toFixed(1)}% — 주당이익이 급증했습니다. 실적 모멘텀이 강력합니다.`);
      bullCount += 1;
    } else if(fin.epsGrowth >= 15){
      lines.push(`EPS 성장률 +${fin.epsGrowth.toFixed(1)}% — 양호한 이익 성장세입니다.`);
    } else if(fin.epsGrowth >= 0){
      lines.push(`EPS 성장률 +${fin.epsGrowth.toFixed(1)}% — 이익이 소폭 증가했습니다.`);
    } else if(fin.epsGrowth > -30){
      lines.push(`EPS 성장률 ${fin.epsGrowth.toFixed(1)}% — 이익이 감소했습니다.`);
    } else {
      lines.push(`EPS 성장률 ${fin.epsGrowth.toFixed(1)}% — 이익이 급감했습니다. 실적 악화 추세에 주의하세요.`);
      bearCount += 1;
    }
  }
...
SXI.advMacro = function(macro){
  ...
  if(macro.dxy && macro.gold){
    if(macro.dxy.trend === 'up' && macro.gold.trend === 'up' && macro.dxy.change5d > 1 && macro.gold.change5d > 1){
      lines.push('⚠️ 달러와 금이 동반 상승 — 일반적 역상관이 깨진 비정상 패턴으로, 지정학적 위기(전쟁·제재)나 글로벌 불확실성 급등을 시사합니다. 극도의 주의가 필요합니다.');
      score -= 15;
    }
  }
...
SXI.advDisclosure = function(keywords){
  ...
  else if(hasSevere) sText += '⚠️ 위험 공시(관리종목/자본잠식/감자 등) 감지. 펀더멘탈 리스크가 매우 높습니다.';
  ...
  const labels = {
    danger:'⛔ 치명적 공시 — 즉시 이탈',
    bearish:'⚠️ 위험 공시 — 투자 부적합',
    warning:'⚡ 주의 공시 — 리스크 확인 필요',
    bullish:'✅ 호재 공시 — 펀더멘탈 개선',
    neutral:'공시 이슈 없음'
  };
...
SXI.overrideSummaryWithDisclosure = function(baseSummary, discItp){
  ...
  if(discItp.tone === 'danger'){
    ...
    s.tone = 'bearish';
    s.stateLine = '⚠️ 위험 공시 감지 — ' + (s.stateLine || '');
    s.mainText = '이 종목에서 관리종목·자본잠식·감자 등 심각한 공시가 감지되었습니다. ' + s.mainText;
    s.actionGuide = '⚠️ 신규 매수 자제. ' + (s.actionGuide || '보유 중이면 비중 축소를 적극 검토하세요.');
...
SXI.overrideSummaryWithShortSelling = function(baseSummary, shortItp, ratio){
  ...
  if(shortItp.tone === 'bearish' && ratio >= 10){
    ...
    s.stateLine = '📉 공매도 잔고 과다 — ' + (s.stateLine || '');
    s.risks.unshift({
      tone:'danger', icon:'📉',
