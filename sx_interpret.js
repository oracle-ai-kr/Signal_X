// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v2.8
//  지표별 해석 + 복합상황 해석 + 시너지 조합 + 종합평(행동가이드) 생성
...
SXI.compositeFlow = function(ind){
  ...
  if(coV!=null && coV>0 && obvT==='up' && eomV2!=null && eomV2>0) notes.push({tone:'bullish',icon:'💰',title:'3중 수급 확인 (Chaikin+OBV+EOM)',text:`Chaikin Osc 양수(매집) + OBV 상승 + EOM 양수(매수 용이) — 세 가지 수급 지표가 동시에 매수를 지지합니다. "스마트 머니"가 적극적으로 물량을 확보하고 있을 가능성이 매우 높으며, 추세 추종 관점에서 강력한 매수 확인 신호입니다.`});
...
SXI.compositeRisk = function(ind){
  ...
  if(abT==='bearish' && rsi>=70) notes.push({tone:'warning',icon:'⚠️',title:'AB 매도 우위인데 RSI 과매수',text:'가격은 아직 높지만 장중 매도 압력이 매수를 압도하고 있습니다. 고점에서의 분배(distribution)가 시작되었을 가능성이 있으며, 윗꼬리 음봉 출현에 주의하세요.'});
  return notes;
};
...
SXI.compositeSynergy = function(ind){
  ...
  if(psyV!=null && psyV>=75 && abT==='bearish')
    notes.push({tone:'danger',icon:'🔴',title:'심리도 과열 + AB 매도 전환 → 고점 분배',text:`심리도 ${psyV.toFixed(0)}%(과매수) 상태에서 AB Ratio가 매도 우위로 전환되었습니다. 과열된 낙관 속에서 장중 매도 압력이 커지고 있어, 스마트 머니가 물량을 던지기 시작했을 가능성이 있습니다. 분할 익절을 강력히 검토하세요.`});
  ...
  if(maArr==='bear' && vhfT==='trending' && eomV!=null && eomV<0 && coV!=null && coV<0 && obvT==='down')
    notes.push({tone:'danger',icon:'☠️',title:'5중 하락 경고 (추세+수급+모멘텀)',text:'MA 역배열 + VHF 추세장 + EOM 매도 용이 + Chaikin 분산 + OBV 하락 — 모든 축이 하락을 가리킵니다. 보유 중이면 즉시 손절하고, 반등은 데드캣 바운스일 가능성이 매우 높습니다.'});
...
SXI.chaikinOsc = function(co){
  ...
  if(v>0 && t==='up') return {label:'강한 매집',tone:'bullish',text:`Chaikin Osc ${v.toFixed(0)} — 양수이면서 상승 중으로, A/D 모멘텀이 가속되고 있습니다. 거래량 흐름이 가격 상승을 적극 뒷받침하는 건강한 매집 구간입니다. "스마트 머니"가 물량을 확보하고 있을 가능성이 높으며, OBV 상승 추세와 함께라면 상승 지속 기대가 높습니다.`};
  ...
  if(v<0 && t==='down') return {label:'강한 분산',tone:'bearish',text:`Chaikin Osc ${v.toFixed(0)} — 음수이면서 하락 중으로, A/D 모멘텀이 약화 가속입니다. 거래량이 하락을 뒷받침하며 "스마트 머니"가 물량을 던지고 있을 가능성이 높습니다. 반등 시 거래량이 동반되지 않으면 데드캣 바운스일 수 있으므로, 음수 → 양수 전환 확인 후에야 매수를 고려하세요.`};
