// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v1.5 <!-- S75 -->
//  S75: 탑바 섹터 표시 + 별 위치 종목명 옆 이동
        ...
        <div class="stock-row" onclick="openAnalysis(${realIdx})">
          <div class="sr-body">
            ...
            <div class="sr-row2">
              ...
              ${s._btScore!=null?`<span class="sr-score-chip" style="color:${s._btScore>=60?'var(--buy)':'var(--text3)'}">BT${s._btScore}</span>`:''}
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _btAct = stock._btAction || '';
        const _as = qs?.score ?? scores?.total ?? 0;
        const _bs = stock._btScore ?? 0;
        ...
        let pgHTML = `<div class="anal-section" style="margin-top:8px">
          <div class="itp-toggle-inline" onclick="const c=document.getElementById('${pgId}');c.classList.toggle('show');this.querySelector('.sb-arrow').textContent=c.classList.contains('show')?'▼':'▶'" style="font-size:11px;color:var(--accent);cursor:pointer;font-weight:700"><span class="sb-arrow">▶</span> 이 결과를 어떻게 활용할까요?</div>
      ...
      ${(()=>{
        ...
        const fa = SXI.failureAnalysis(_btD, indicators, qs, currentTF);
        if(!fa) return '';
        ...
        let faHTML = `<div class="anal-section" style="margin-top:8px">
          <div class="itp-toggle-inline" onclick="const c=document.getElementById('${faId}');c.classList.toggle('show');this.querySelector('.sb-arrow').textContent=c.classList.contains('show')?'▼':'▶'" style="font-size:11px;color:var(--accent);cursor:pointer;font-weight:700"><span class="sb-arrow">▶</span> 실패 분석 <span style="font-size:9px;font-weight:600;color:${riskColor};margin-left:4px">${fa.riskProfile.label}</span></div>
    ...
    ${(()=>{
      ...
      const _baseTh = (typeof _getEffectiveTh!=='undefined') ? _getEffectiveTh(currentTF) : null;
      ...
      if(_raEnabled && _ra && _ra.buyThAdj !== 0){
        ...
        if(_raGuide){
          _raHTML += `<div style="margin-top:4px">
            <div class="itp-toggle-inline" onclick="const c=document.getElementById('${_raGuideId}');c.classList.toggle('show');this.querySelector('.sb-arrow').textContent=c.classList.contains('show')?'▼':'▶'" style="font-size:10px;color:var(--accent);cursor:pointer;font-weight:600"><span class="sb-arrow">▶</span> 적응형 파라미터 상세</div>
    ...
    <div class="anal-section">
      ...
      ${(()=>{
        ...
        return `<div class="itp-toggle-inline" onclick="const el=document.getElementById('${ciId}');el.classList.toggle('show')"><span class="sb-arrow">▶</span> ${ci.label}</div>
      ...
      ${(()=>{
        ...
        return `<div class="itp-toggle-inline" onclick="const el=document.getElementById('${siId}');el.classList.toggle('show')"><span class="sb-arrow">▶</span> ${si.label}</div>
        ...
        if(el && d && d.acc_month){
          ...
          if(itpArea && typeof SXI!=='undefined' && SXI.advSettleMonth){
            ...
            if(smi){
              ...
              itpArea.innerHTML = `<div class="itp-toggle-inline" onclick="const el=document.getElementById('${smId}');el.classList.toggle('show')"><span class="sb-arrow">▶</span> ${smi.label}</div>
...
function renderSectorRadarCard(radarGuide) {
  ...
  if (g.watchlistTexts.length > 0) {
    html += `<div style="margin-top:4px"><div style="font-size:10px;font-weight:700;color:var(--accent);margin-bottom:4px">📌 내 관심종목 섹터 현황</div>`;
...
function renderCorrelationCard(corrGuide, corrData) {
  ...
  // 섹터 분포
  if (g.sectorText) {
    html += `<div style="font-size:10px;padding:6px 8px;border-radius:6px;background:var(--surface2);margin-top:6px;color:var(--text2)">📊 ${g.sectorText}</div>`;
  }
