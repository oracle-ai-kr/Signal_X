...
let _optMinTrades = 0; // [S244] 최소 거래수 필터 — 기본 OFF (빈칸=OFF=0, 추천값은 input placeholder로 안내)
...
const _OPT_MODE_WEIGHTS = {
  profit:   { pnl:30, trades:5, mdd:15, wr:35, pf:15 },
  balanced: { pnl:25, trades:5, mdd:15, wr:40, pf:15 },
  safe:     { pnl:10, trades:5, mdd:40, wr:30, pf:15 }
};
...
const _OPT_MODE_TIEBREAK_DEFAULTS = {
  profit:   { pnl:1, trades:4, mdd:3, wr:2, pf:5 },
  balanced: { pnl:2, trades:4, mdd:3, wr:1, pf:5 },
  safe:     { pnl:3, trades:4, mdd:1, wr:2, pf:5 }
};
...
function _optSortScore(bt, mode){
  ...
  const nTarget = _OPT_TRUST_N[tfKey] || 20;
...
function _optSortCompare(a, b, mode){
  ...
  const _ntA = _OPT_TRUST_N[a._tf || a.tf || 'day'] || 20;
  const _ntB = _OPT_TRUST_N[b._tf || b.tf || 'day'] || 20;
  ...
  overlay.innerHTML = `<div class="opt-panel">
    ...
    <div class="opt-body">
      ...
      <div class="opt-section" id="optControlSection">
        ...
        <!-- [v3.18 PF-AXIS-INPUT] 거래수 토글 → 입력 박스로 변경
             사유: 사용자가 능동적으로 임계값 설정 (10건 고정 토글 → 빈칸 가능 + 직접 입력)
                   비워두면 필터 OFF, 숫자 입력 시 해당 거래수 미만 결과 제외
             추천 안내: TF별 적정선 간략 표시 (일봉 20 / 주봉·월봉 5~10) -->
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:6px 10px;background:var(--surface2,#f5f5f5);border-radius:6px;border:1px solid var(--border,#e0e0e0)">
          <div style="flex:1">
            <span style="font-size:10px;color:var(--text,#222);font-weight:600">거래수 <input type="number" id="optMinTradesInput" value="" min="1" max="999" step="1" placeholder="-" style="width:42px;padding:1px 3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center;font-weight:600" onchange="_optUpdateMinTrades()">건 미만 결과 제외</span><br>
            <span style="font-size:9px;color:var(--text3,#999)">비우면 OFF · 추천: 일봉 20 / 주봉·월봉 5~10 / 분봉 25~30</span>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:6px 10px;background:var(--surface2,#f5f5f5);border-radius:6px;border:1px solid var(--border,#e0e0e0)">
        ...
        <div id="optWeightArea" style="display:none;padding:8px 10px;margin-bottom:8px;background:var(--surface2,#f5f5f5);border-radius:8px;border:1px solid var(--border,#e0e0e0)">
          ...
          <div style="display:grid;grid-template-columns:50px 28px 1fr 1fr 1fr 1fr 1fr 30px;gap:2px 3px;align-items:center;font-size:9px;text-align:center">
            ...
            <input type="number" id="optTB_profit_trades" value="4" min="1" max="5" step="1" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_profit_mdd" value="3" min="1" max="5" step="1" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_profit_wr" value="2" min="1" max="5" step="1" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <input type="number" id="optCW_profit_pnl" value="30" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_profit_trades" value="5" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_profit_mdd" value="15" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_profit_wr" value="35" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_profit_pf" value="15" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <span></span>
            <span style="font-size:8px;color:var(--text3,#999)">비중</span>
            <input type="number" id="optCW_balanced_pnl" value="25" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <input type="number" id="optCW_balanced_mdd" value="15" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <input type="number" id="optCW_balanced_pf" value="15" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <span></span>
            <span style="font-size:8px;color:var(--text3,#999)">비중</span>
            <input type="number" id="optCW_safe_pnl" value="10" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <input type="number" id="optCW_safe_pf" value="15" min="0" max="100" step="5" style="width:100%;padding:2px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
  ...
  try {
    ...
    for(let round = 1; round <= repeatCount; round++){
      ...
      for(let si = 0; si < unlocked.length; si++){
        ...
        // 이 파라미터의 최적값 추출 (TOP1)
        const merged = [];
        const addM = (res, regime) => { if(!res) return; Object.entries(res).forEach(([tf2,arr])=>{ if(Array.isArray(arr)) arr.filter(r=>r.bt&&r.bt.totalTrades>=_optMinTrades&&r.bt.winRate>=_optMinWinRate).forEach(r=>merged.push({...r,regime,tf:tf2,bt:{...r.bt,_tf:tf2}})); }); };
...
//   기존: optMinTradesToggle (체크박스, OFF=3 / ON=10 고정)
//   변경: optMinTradesInput (숫자 입력, 비우면 OFF=0 / 입력값 미만 제외)
//   _optMinTrades 0이면 모든 결과 통과 (`r.bt.totalTrades >= 0` 항상 참)
function _optUpdateMinTrades(){
  _optVib(10);
  const inp = document.getElementById('optMinTradesInput');
  const v = inp ? inp.value.trim() : '';
  if(v === '' || isNaN(parseInt(v)) || parseInt(v) <= 0){
    _optMinTrades = 0; // OFF
    toast('거래수 필터 OFF');
  } else {
    _optMinTrades = parseInt(v);
    toast(`거래수 ${_optMinTrades}건 미만 제외`);
  }
}
...
async function _optSetModeCore(mode, needsConfirm){
  ...
  const mLabels = {profit:'🔥 수익형(상승)', balanced:'⚖️ 안정형(횡보)', safe:'🛡️ 보수형(하락)'};
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    ...
    const add = (res, regime) => {
      ...
      // [v3.18 PF-TRUST] bt에 _tf 주입 — _optSortScore에서 TF별 N_적정 사용 위함
      res.filter(r=>r.bt&&r.bt.totalTrades>=_optMinTrades&&r.bt.winRate>=_optMinWinRate).forEach(r=>merged.push({params:r.params,bt:{...r.bt,_tf:tf},regime,tf,code,_stockCount:r._stockCount}));
    };
    add((tfResOff||{})[tf], 'OFF');
    add((tfResOn||{})[tf], 'ON');
...
function _optSyncFromMarket(m){
  try {
    ...
    if(!document.getElementById('optStockChips')) {
      // 옵티마이저 모달 닫힌 상태 — _optMarket만 변수 갱신 (다음 모달 열 때 적용됨)
